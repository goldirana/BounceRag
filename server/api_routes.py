...
#     firestore_chat_history.add_ai_message(response)
...
#     # Extract text response and images separately
#     text_response = response_data["response"]
#     image_urls = [f"/static/{os.path.basename(img_path)}" for img_path in response_data.get("image_paths", [])]
...
async def search_vb(request: QueryRequest):
    ...
    print("DEBUG RESPONSE DATA:", response_data)  # Check the response format
